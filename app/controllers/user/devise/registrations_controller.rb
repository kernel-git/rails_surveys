...
class User::Devise::RegistrationsController < Devise::RegistrationsController
  ...
  def new
    print("Ping from user/devise/registration#new")
  ...
  # POST /resource
  def create
    print("Ping from user/devise/registration#create")
    params[:user][:position_age] = 1
    params[:user][:client_id] = params[:client_id]
    super
  end
