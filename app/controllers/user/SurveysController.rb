class User::SurveysController < ApplicationController
  ...
  def index
    @avaible_surveys = Survey.filter_avaible_by_assigned_user_id(current_user.id).page(params[:page])
    @conducted_surveys = Survey.filter_conducted_by_assigned_user_id(current_user.id)
  end
  def attempt
    @survey = Survey.filter_avaible_by_assigned_user_id(current_user.id).find(params[:id])
    @answer = Answer.new
  ...
  def conduct
    ...
    params[:answers].each do |question_id, answer_data|
      ...
      new_answer.user = User.find(current_user.id)
    ...
    begin 
      ...
      relation = SurveyUserRelation.where(survey_id: params[:id], user_id: current_user.id).first
